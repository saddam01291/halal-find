...
export function SearchClient({ initialPlaces, initialQuery }: SearchClientProps) {
    ...
    // Hydrate with initial server-side places
    const [places, setPlaces] = useState<DbPlace[]>(initialPlaces);
    const [loading, setLoading] = useState(false);
    const [error, setError] = useState<string | null>(null);

    // Keep track of whether we've done the initial client-side fetch (for location)
    const [hasAttemptedLocationFetch, setHasAttemptedLocationFetch] = useState(false);

    useEffect(() => {
        // If query changes from server or we need to update based on userCoords
        // We only fetch on client if it's a NEW client-side search OR if we have userCoords
        
        // If no user coords and query == initialQuery, we can just rely on the initialServerPlaces for now
        if (!userCoords && query === initialQuery && !hasAttemptedLocationFetch && initialPlaces.length > 0) {
            return;
        }

        const fetchPlaces = async (retryCount = 0) => {
            console.log(`Search: Fetching places for query: "${query}" (retry: ${retryCount})`);
            setLoading(true);
            setError(null);
            ...
            try {
                ...
                const data = query 
                    ? await searchPlaces(query, userCoords || undefined) 
        ...
        fetchPlaces();
    }, [query, userCoords, locationStatus]);
